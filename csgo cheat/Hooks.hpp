...
public:
	...
	static void __fastcall hkRunCommand(void* ecx, void* edx, C_BasePlayer* player, C_UserCmd* m_pcmd, C_MoveHelper* move_helper);
	...
	static int __fastcall hkSetupVelocity(LPVOID pEcx, uint32_t);
	//static void __fastcall hkAccumulateLayers(C_BasePlayer* player, uintptr_t edx, DWORD* setup, Vector* pos, float time, Quaternion* q);
