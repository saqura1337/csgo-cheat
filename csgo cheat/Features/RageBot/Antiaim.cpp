...
void C_AntiAim::Instance()
{
	...
	if (m_AntiAimConditions[nCommand % MULTIPLAYER_BACKUP])
	{
		...
		if (m_AntiAimConditions[nCommand % MULTIPLAYER_BACKUP] == ANTIAIM_FULL)
		{
			...
			else
			{
				...
				if (GetAsyncKeyState(g_cfg->m_aManualLeft->m_iKeySelected))
					m_iManualSide = -1;

				if (GetAsyncKeyState(g_cfg->m_aManualBack->m_iKeySelected))
					m_iManualSide = 0;

				if (GetAsyncKeyState(g_cfg->m_aManualRight->m_iKeySelected))
					m_iManualSide = 1;

				flFinalYaw = Math::NormalizeAngle(flFinalYaw + (m_iManualSide < INT_MAX ? 90.0f * m_iManualSide : g_cfg->m_iYawAddOffset));
			}
		}

		if (g_PacketManager->GetModifablePacket())
		{
			if (g_Tools->IsBindActive(g_cfg->m_aInverter))
				m_iDesyncSide = 1;
			else
				m_iDesyncSide = -1;

			m_bShouldUseAlternativeSide = false;
			if (g_cfg->m_aInverterConditions[0] && (g_sdk.local->m_fFlags() & FL_ONGROUND) && g_sdk.local->m_vecVelocity().Length2D() < 23.40f && g_sdk.local->m_vecVelocity().z == 0.0f)
			{
				m_iFinalSide = -m_iFinalSide;
				m_bShouldUseAlternativeSide = true;
			}
			else if (g_sdk.local->m_vecVelocity().Length2D() > 23.40f && g_sdk.local->m_vecVelocity().z == 0.0f && (g_sdk.local->m_fFlags() & FL_ONGROUND))
			{
				if (g_cfg->m_aInverterConditions[1])
				{
					m_iFinalSide = -m_iFinalSide;
					m_bShouldUseAlternativeSide = true;
				}
			}
			else if (!(g_sdk.local->m_fFlags() & FL_ONGROUND))
			{
				if (g_cfg->m_aInverterConditions[2])
				{
					m_iFinalSide = -m_iFinalSide;
					m_bShouldUseAlternativeSide = true;
				}
			...
			{
				if (m_bSwitch)
					flFinalYaw += g_cfg->m_iJitterAmount;
				else
					flFinalYaw -= g_cfg->m_iJitterAmount;
			}
		...
		g_PacketManager->GetModifableCommand()->m_angViewAngles.yaw = Math::NormalizeAngle(flFinalYaw + g_cfg->m_iYawAddOffset);
	}
	...
	if (!g_PacketManager->GetModifablePacket())
	{
		...
		if (m_iDesyncSide > 0)
			flDesyncAmount = g_cfg->m_iRightFakeLimit * 2.0f;
		else
			flDesyncAmount = g_cfg->m_iLeftFakeLimit * 2.0f;
...
float C_AntiAim::GetTargetYaw()
{
	...
	for (int32_t i = 1; i < 65; i++)
	{
		...
		if (!pPlayer || !pPlayer->IsPlayer() || !pPlayer->IsAlive() || pPlayer->m_iTeamNum() == g_sdk.local->m_iTeamNum() || pPlayer->IsDormant())
			continue;

		if (pPlayer->m_fFlags() & FL_FROZEN)
			continue;

		float_t flDistanceToPlayer = g_sdk.local->m_vecOrigin().DistTo(pPlayer->m_vecOrigin());
	...
	return Math::CalcAngle(g_sdk.local->GetAbsOrigin() + g_sdk.local->m_vecViewOffset(), pFinalPlayer->GetAbsOrigin()).yaw + 180.0f;
}
...
void C_AntiAim::SlowWalk(bool bForcedWalk)
{
	...
	if (!g_Tools->IsBindActive(g_cfg->m_aSlowwalk))
		bCanSlowwalk = false;

	if ((g_sdk.local->m_fFlags() & FL_FROZEN) || (g_sdk.local->m_bGunGameImmunity()) /*|| ( *( g_Globals.m_Interfaces.m_GameRules ) )->IsFreezePeriod( )*/)
		bCanSlowwalk = false;

	float_t flMovementLength = g_sdk.local->m_vecVelocity().Length2D();
	if ((!bCanSlowwalk && !bForcedWalk) || !(g_sdk.local->m_fFlags() & FL_ONGROUND))
		return;
...
{
	if (!g_cfg->m_bAntiAim || !((g_sdk.local->m_fFlags() & FL_ONGROUND)))
		return;
...
{
	if (!g_cfg->m_bAntiAim || g_PacketManager->GetModifableCommand()->m_nButtons & (IN_MOVELEFT | IN_MOVERIGHT | IN_BACK | IN_FORWARD))
		return;

	float_t flVelocityLength = g_sdk.local->m_vecVelocity().Length2D();
	if (flVelocityLength > 5.0f)
		return;

	float_t flSpeed = 1.1f;
	if ((g_PacketManager->GetModifableCommand()->m_nButtons & IN_DUCK) || g_sdk.packet.m_bFakeDuck)
		flSpeed *= 3.3333334f;
...
int32_t C_AntiAim::GetAntiAimConditions()
{
	...
	if ((g_sdk.local->m_fFlags() & FL_FROZEN) /*|| ( *g_Globals.m_Interfaces.m_GameRules )->IsFreezePeriod( )*/)
		return 0;

	if (g_sdk.local->GetMoveType() == MOVETYPE_LADDER)
	{
	...
	if (g_PacketManager->GetModifableCommand()->m_nButtons & IN_ATTACK)
	{
		...
		if (g_sdk.local->CanFire(nShiftAmount, bIsRevolver))
			return 0;
	}
