...
{
	m_data = {};
	const auto weapon = g_sdk.local->m_hActiveWeapon( ).Get( );
	if ( !weapon
	...
	const auto eye_pos = g_sdk.local->GetShootPosition( );
	...
	g_interfaces.trace->TraceHull(
		...
		MASK_SOLID | CONTENTS_CURRENT_90, g_sdk.local, COLLISION_GROUP_NONE, &trace
	);
	...
	m_data.predict(
		...
		direction * ( std::clamp< float >(
			...
			) * ( throw_strength * 0.7f + 0.3f ) ) + g_sdk.local->m_vecVelocity( ) * 1.25f,
		TICKS_TO_TIME( g_sdk.local->m_nTickBase( ) ),
		0
	);
}