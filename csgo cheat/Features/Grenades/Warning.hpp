...
public:
	struct data_t {
		...
		bool physics_simulate( ) {
			...
			static const auto sv_gravity = g_sdk.convars.m_SvGravity;

			const auto new_velocity_z = m_velocity.z - ( sv_gravity->GetFloat( ) * 0.4f ) * g_interfaces.globals->m_intervalpertick;

			const auto move = Vector(
				m_velocity.x * g_interfaces.globals->m_intervalpertick,
				m_velocity.y * g_interfaces.globals->m_intervalpertick,
				( m_velocity.z + new_velocity_z ) / 2.f * g_interfaces.globals->m_intervalpertick
			);
		...
		void physics_trace_entity( const Vector& src, const Vector& dst, std::uint32_t mask, trace_t& trace ) {
			...
			g_interfaces.trace->TraceLine( src, dst, mask, m_owner, m_collision_group, &trace );
		}
		void physics_push_entity( const Vector& push, trace_t& trace ) {
			...
			static const auto weapon_molotov_maxdetonateslope = g_interfaces.cvar->FindVar( _S( "weapon_molotov_maxdetonateslope" ) );
		...
		void perform_fly_collision_resolution( trace_t& trace ) {
			...
			if ( trace.plane.normal.z > 0.7f ) {
				...
				if ( speed_sqr < 400.f ) {
					...
					physics_push_entity( velocity * ( ( 1.f - trace.fraction ) * g_interfaces.globals->m_intervalpertick ), trace );
				}
			}
			else {
				m_velocity = velocity;
		...
		void predict( const Vector& origin, const Vector& velocity, float throw_time, int offset ) {
			...
			case WEAPON_MOLOTOV:
			case WEAPON_INCGRENADE:
				static const auto molotov_throw_detonate_time = g_interfaces.cvar->FindVar( _S( "molotov_throw_detonate_time" ) );
		...
		bool draw( ) const {
			if ( m_path.size( ) <= 1u
				|| TICKS_TO_TIME( g_sdk.local->m_nTickBase( ) ) >= m_ExpireTime )
				return false;
			...
			for ( auto i = 1u; i < m_path.size( ); ++i ) {
				...
				{
					g_Render->RenderLine( prev_screen.x, prev_screen.y, cur_screen.x, cur_screen.y, Color( g_cfg->m_GrenadeWarning ), 1.0f );
				
					if ( i == m_path.size( ) - 1 )
						g_Render->RenderCircle3D( std::get< Vector >( m_path.at( i ) ), 32, 171, Color( g_cfg->m_GrenadeWarning ) );
				}
			...
			if ( !g_interfaces.debug_overlay->ScreenPosition( Vector( vecTimerPosition.x, vecTimerPosition.y, vecTimerPosition.z ), vecScreenPosition ) )
			{
				std::string szIcon = "";
				switch ( m_index )
				{
				...
				ImVec2 vecTextSize = g_sdk.fonts.m_BigIcons->CalcTextSizeA( 20.0f, FLT_MAX, NULL, szIcon.c_str( ) );
				g_Render->RenderText( szIcon, ImVec2( vecScreenPosition.x - vecTextSize.x / 2 + 1, vecScreenPosition.y - vecTextSize.y / 2 ), Color::White, false, true, g_sdk.fonts.m_BigIcons );
				
				if ( g_cfg->m_GrenadeTimers )
				{
					float_t flPercentage = ( m_ExpireTime - TICKS_TO_TIME( g_sdk.local->m_nTickBase( ) ) ) / ( m_ExpireTime - m_SourceTime ) ;
