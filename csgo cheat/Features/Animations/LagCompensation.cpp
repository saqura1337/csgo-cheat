...
void C_LagCompensation::Instance(ClientFrameStage_t Stage)
{
	...
	for (int32_t iPlayerID = 1; iPlayerID <= g_interfaces.globals->m_maxclients; iPlayerID++)
	{
		...
		if (!pPlayer || !pPlayer->IsPlayer() || !pPlayer->IsAlive() || pPlayer->m_iTeamNum() == g_sdk.local->m_iTeamNum())
		{
			...
			g_sdk.m_CachedPlayerRecords[iPlayerID].clear();
			continue;
		}
		...
		if (g_sdk.m_CachedPlayerRecords[iPlayerID].empty())
			bHasPreviousRecord = false;
		else if (TIME_TO_TICKS(fabs(pPlayer->m_flSimulationTime() - g_sdk.m_CachedPlayerRecords[iPlayerID].back().m_SimulationTime)) <= 17)
		...
		g_sdk.m_CachedPlayerRecords[iPlayerID].emplace_back(LagRecord);
		while (g_sdk.m_CachedPlayerRecords[iPlayerID].size() > 32)
			g_sdk.m_CachedPlayerRecords[iPlayerID].pop_front();
	}
}
...
void C_LagCompensation::InterpolatePlayers()
{
	for (int32_t iPlayerID = 1; iPlayerID <= g_interfaces.globals->m_maxclients; iPlayerID++)
	{
		...
		*(float*)(*(DWORD*)(*(DWORD*)((DWORD)(pPlayer)+0x24) + 8) + 0x24) = 14.0f / g_interfaces.globals->m_intervalpertick;
		*(float*)(*(DWORD*)(*(DWORD*)((DWORD)(pPlayer)+0x24) + 0x44) + 0x24) = 14.0f / g_interfaces.globals->m_intervalpertick;
	}
}
...
{
	float_t flUpdateRate = std::clamp(g_sdk.convars.m_ClUpdateRate->GetFloat(), g_sdk.convars.m_SvMinUpdateRate->GetFloat(), g_sdk.convars.m_SvMaxUpdateRate->GetFloat());
	float_t flLerpRatio = std::clamp(g_sdk.convars.m_ClInterpRatio->GetFloat(), g_sdk.convars.m_SvClientMinInterpRatio->GetFloat(), g_sdk.convars.m_SvClientMaxInterpRatio->GetFloat());
	return std::clamp(flLerpRatio / flUpdateRate, g_sdk.convars.m_ClInterp->GetFloat(), 1.0f);
}