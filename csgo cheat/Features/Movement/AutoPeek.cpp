...
void C_AutoPeek::Instance( )
{
	...
	else if ( g_Tools->IsBindActive( g_cfg->m_aAutoPeek ) )
	{
		if ( !m_bTurnedOn )
		{
			m_bTurnedOn = true;
			...
			m_flAnimationTime = g_interfaces.globals->m_realtime + .1f;

			m_vecStartPosition = g_sdk.local->GetAbsOrigin( );
		}
	}
	else if ( m_bTurnedOn )
	{
	...
	if ( !m_bRetreat )
		if ( g_PacketManager->GetModifableCommand( )->m_nButtons & IN_ATTACK )
			if ( g_sdk.local->m_hActiveWeapon( ) )
				if ( g_sdk.local->m_hActiveWeapon( )->IsGun( ) )
	...
	auto vecDifference = g_sdk.local->GetAbsOrigin( ) - m_vecStartPosition;
	...
	g_interfaces.engine->GetViewAngles( &angWishAngles );
...
void C_AutoPeek::DrawCircle( )
{
	...
	{
		flAnimationProgress = 1.0f - ( ( m_flAnimationTime - g_interfaces.globals->m_realtime ) / .1f );
		if ( m_bNegativeSide )
