...
void C_DrawModel::Instance()
{
	...
	if (!m_PreviewTexture)
	{
		g_interfaces.material_system->BeginRenderTargetAllocation();

		m_PreviewTexture = g_interfaces.material_system->CreateNamedRenderTargetTextureEx(
			_S("Preview"),
			350, 575,
			RT_SIZE_NO_CHANGE,
			g_interfaces.material_system->GetBackBufferFormat(),
			MATERIAL_RT_DEPTH_SHARED,
			TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT,
			CREATERENDERTARGETFLAGS_HDR
		);

		g_interfaces.material_system->FinishRenderTargetAllocation();
	}

	if (!m_CubemapTexture)
		m_CubemapTexture = g_interfaces.material_system->FindTexture(_S("editor/cubemap.hdr"), _S(TEXTURE_GROUP_CUBE_MAP));

	auto CreateModel = reinterpret_cast<void(__thiscall*)(void*)>(g_sdk.address_list.m_CreateModel);
	if (!m_PreviewModel)
	{
		m_PreviewModel = static_cast<C_MergedMDL*>(g_interfaces.mem_alloc->Alloc(0x75C));
		CreateModel(m_PreviewModel);

		m_PreviewModel->SetMDL(_S("models/player/custom_player/uiplayer/animset_uiplayer.mdl"));
		m_PreviewModel->SetMergedMDL(_S("models/player/custom_player/legacy/tm_professional_vari.mdl"));
		m_PreviewModel->SetMergedMDL(_S("models/weapons/w_axe.mdl"));

		m_PreviewModel->SetSequence(32, false);
		m_PreviewModel->SetupBonesForAttachmentQueries();
	}

	m_PreviewModel->RootMDL.flTime += g_interfaces.globals->m_frametime / 2.0f;
	...
	CMatRenderContextPtr pRenderContext(g_interfaces.material_system);
	...
	g_interfaces.render_view->Push3DView(pRenderContext, m_ViewSetup, VIEW_CLEAR_COLOR | VIEW_CLEAR_DEPTH | VIEW_CLEAR_STENCIL, m_PreviewTexture, dummyFrustum);
	...
	Math::AngleMatrix(QAngle(0, g_sdk.model.m_flModelRotation, 0), matPlayerView, Vector(0, 0, 0));

	g_interfaces.model_render->SuppressEngineLighting(true);
	...
	g_sdk.model.m_bDrawModel = false;

	g_interfaces.model_render->SuppressEngineLighting(false);

	g_interfaces.render_view->PopView(pRenderContext, dummyFrustum);
...
{
	return ((void(__thiscall*)(void*))(g_sdk.address_list.m_SetupBonesForAttachmentQueries))(this);
}

bool& C_MaterialSystem::DisableRenderTargetAllocationForever()
{
	return *reinterpret_cast<bool*>(reinterpret_cast<std::uintptr_t>(this) + g_sdk.address_list.m_DisableRenderTargetAllocationForever);
}

void C_MergedMDL::SetMergedMDL(const char* szModelPath, CCustomMaterialOwner* pCustomMaterialOwner, void* pProxyData)
{
	return ((void(__thiscall*)(void*, const char*, CCustomMaterialOwner*, void*, bool))(g_sdk.address_list.m_SetMergedMDL))(this, szModelPath, pCustomMaterialOwner, pProxyData, false);
}