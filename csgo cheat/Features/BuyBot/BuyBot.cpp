...
void C_BuyBot::OnCreateMove()
{
	...
	if (!g_cfg->m_bBuyBotEnabled)
		return;

	switch (g_cfg->m_BuyBotPrimaryWeapon)
	{
	...
	if (g_sdk.local->m_hActiveWeapon().Get())
		if (g_sdk.local->m_hActiveWeapon().Get()->m_iItemDefinitionIndex() == WEAPON_AWP)
			if (g_cfg->m_bBuyBotKeepAWP)
				strBuyCommand = "";

	switch (g_cfg->m_BuyBotSecondaryWeapon)
	{
