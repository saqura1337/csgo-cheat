...
struct C_RageSettings
{
	...
	bool m_Hitboxes[6] = { false, false, false, false, false, false }; // head, chest, arms, pelvis, stomach, legs
...
public:
	...
	SETTING(bool, m_bStaticLegs, false);
	...
	SETTING(bool, m_bKillsound, false);
	SETTING(bool, m_bPanoramaBlur, false);
	...
	SETTING(bool, bSunSetMode, false);
	SETTING(bool, m_bDamageMarker, false);
	SETTING(bool, m_bBombTimer, false);
	...
	SETTING(int32_t, m_iSunSetX, 0);
	SETTING(int32_t, m_iSunSetY, 0);
	SETTING(int32_t, m_iSunSetZ, 0);
	...
	SETTING(C_KeyBind, m_aEdgeJump, C_KeyBind());
	...
	SETTING(C_KeyBind, m_aFakeFlick, C_KeyBind());

	// 0 - enemy
	// 1 - teammate
	// 2 - localplayer
	SETTING(C_PlayerSettings, local, C_PlayerSettings());
	...
	// 8 - landing bob
	// 9 - hand shaking
	std::array < bool, 11 > m_aWorldRemovals = { };
	...
	std::array <item_setting, 1000> m_aSkins = {};
...
inline C_Settings* g_cfg = new C_Settings();