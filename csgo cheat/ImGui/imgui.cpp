...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddText({ child_window->DC.CursorPos.x + 1 ,ImGui::GetWindowPos().y - 22 }, GetColorU32(ImVec4(255 / 255.0f, 255 / 255.0f, 255 / 255.0f, g.Style.Alpha)), name);

    parent_window->DrawList->AddRectFilledMultiColor(ImVec2(cursor.x - 12, cursor.y - 12), ImVec2(pos.x + 120.f, (cursor.y - 12) + 2), col2, col, col, col2);
    parent_window->DrawList->AddRectFilledMultiColor(ImVec2(pos.x + size.x - 135.f, cursor.y - 12), ImVec2(cursor.x + width + 4.f, (cursor.y - 12) + 2), col, col2, col2, col);
...
void ImGui::EndChild()
{
    ...
    else
    {
        ...
        parent_window->DrawList->AddRectFilled({ window->Pos.x  ,window->Pos.y }, { window->Pos.x + window->Size.x ,window->Pos.y + window->Size.y }, GetColorU32(ImVec4(3 / 255.0f, 16 / 255.0f, 32 / 255.0f, 170 / 255.0f)), 6.f);
        ...
        //if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            if (!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip))
            {
                ...
                //  ImGui::SetCursorPos(ImVec2(1, 1));
                 // ImGui::Image(g_Menu->GetTexture2(), ImVec2(796, 499));
                window->DrawList->AddRectFilled({ window->Pos.x + 1,window->Pos.y + 1 }, { window->Pos.x + window->Size.x,window->Pos.y + 47 }, ImColor(3, 15, 27, alpha), 0, ImDrawCornerFlags_Top);
                window->DrawList->AddRectFilled({ window->Pos.x + 1 ,window->Pos.y + 43 }, (window->Pos + ImVec2(0, 43)) + (window->Size - ImVec2(0, 43)), ImColor(3, 16, 32, alpha - 100), 0, ImDrawCornerFlags_Bot);

                const auto start_col = ImGui::GetStyleColorVec4(ImGuiCol_SliderGrabActive);

                //window->DrawList->AddRectFilledMultiColor({ window->Pos.x + 1 ,window->Pos.y + 43 }, window->Pos + ImVec2(window->Size.x, 60), ImColor(3, 16, 32, 100), ImColor(3, 15, 27, 60), ImColor(3, 16, 32, 50), ImColor(3, 15, 27, 100));
                ...
                //  //  draw->AddText(ImVec2(wp.x + 30, wp.y + 14), ImColor(102, 153, 204, alpha), "reborn hack");
                //    ImGui::SetCursorPos(ImVec2(30, 0 - 9));
               ...
               // PushFont(ImGui::GetIO().Fonts->Fonts[5]);
                //draw->AddText(ImVec2(wp.x + 60, wp.y + 11), ImColor(102, 153, 204, alpha), g_Sdk.m_szUzername.c_str( ));
               // PopFont();
            ...
            else
            {
                ...
                {
                    draw->AddRectFilled(window->Pos, window->Pos + window->Size, ImColor(11, 13, 18, 100), 4.0F, ImDrawCornerFlags_Bot);
...
bool ImGui::subtub(const char* label, bool selected)
{
    ...
    ImVec2 size = ImGui::CalcItemSize(ImVec2(120, 35), label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    //{
       ...
       // window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Max.y), ImVec2(bb.Max.x, bb.Max.y - 3), ImColor(50, 142, 217, 255));
   // }

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 0 + (size.x / 2 - label_size.x / 2), bb.Min.y + (size.y / 2 - label_size.y / 2)), ImColor(170, 170, 170, int(255)), label);
    if (selected)
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 0 + (size.x / 2 - label_size.x / 2), bb.Min.y + (size.y / 2 - label_size.y / 2)), ImColor(50, 142, 217, int((255))), label);
