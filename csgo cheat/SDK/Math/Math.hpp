...
namespace Math
{
	...
	__forceinline float AngleDiff(float destAngle, float srcAngle)
	{
		...
		for (; srcAngle < -180.0; srcAngle = srcAngle + 360.0)
			;
		for (; destAngle > 180.0; destAngle = destAngle - 360.0)
			;
		for (; destAngle < -180.0; destAngle = destAngle + 360.0)
			;
		for (difference = destAngle - srcAngle; difference > 180.0; difference = difference - 360.0)
			;
	...
	__forceinline void angle_vectors(const QAngle& angles, Vector* forward, Vector* right, Vector* up)
	{
		...
		sin_cos(M_PI / 180.0f * angles.pitch, &sp, &cp);
		sin_cos(M_PI / 180.0f * angles.yaw, &sy, &cy);
		sin_cos(M_PI / 180.0f * angles.roll, &sr, &cr);
	...
	__forceinline void angle_vectors(const Vector& angles, Vector& forward)
	{
		float sp, sy, cp, cy;
