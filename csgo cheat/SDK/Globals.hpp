...
typedef void(__thiscall* AccumulateLayers_t)(C_BasePlayer*, uintptr_t, DWORD*, Vector*, float, Quaternion*);
typedef void(__thiscall* RunCommand_t)(void*, C_BasePlayer*, C_UserCmd*, C_MoveHelper*);
typedef int(__thiscall* SetupVelocity_t)(LPVOID);
...
public:
	...
	struct
	{
		...
		uint8_t* m_PhysicsRunThink = nullptr;
	} address_list;
	...
	struct
	{
		...
		HMODULE vgui_dll = nullptr;
		HMODULE vgui2_dll = nullptr;
		HMODULE tier_dll = nullptr;
		HMODULE mat_sys_dll = nullptr;
		HMODULE localize_dll = nullptr;
		HMODULE shader_dll = nullptr;
		HMODULE data_cache_dll = nullptr;
		HMODULE vstd_dll = nullptr;
		HMODULE physics_dll = nullptr;
		HMODULE file_system_dll = nullptr;
		HMODULE studio_render_dll = nullptr;
	} modules;
	...
	struct
	{
		struct
		{
			...
			AccumulateLayers_t accumulate_layers = NULL;
			SetupVelocity_t setup_velocity = NULL;
		} originals;
	} hooks;
...
{
public:
	C_ClientEntityList* entity_list = nullptr;
	C_BaseClientDLL* client = nullptr;
	C_EngineClient* engine = nullptr;
	C_EngineSound* engine_sound = nullptr;
	C_EngineTrace* trace = nullptr;
	C_Localize* localize = nullptr;
	C_Prediction* prediction = nullptr;
	C_MoveHelper* move_helper = nullptr;
	C_GameMovement* game_movement = nullptr;
	C_GlobalVarsBase* globals = nullptr;
	C_Surface* surface = nullptr;
	C_DebugOverlay* debug_overlay = nullptr;
	C_FileSystem* file_system = nullptr;
	C_Cvar* cvar = nullptr;
	C_ClientState* client_state = nullptr;
	C_Input* input = nullptr;
	C_GameRules** game_rules = nullptr;
	C_ViewRenderBeams* view_render_beams = nullptr;
	C_PhysicsSurfaceProps* prop_physics = nullptr;
	C_MDLCache* mdl_cache = nullptr;
	C_Panel* vgui_panel = nullptr;
	C_MaterialSystem* material_system = nullptr;
	C_ModelInfoClient* model_info = nullptr;
	C_ModelRender* model_render = nullptr;
	C_StudioRender* studio_render = nullptr;
	C_GameEventManager* event_manager = nullptr;
	C_RenderView* render_view = nullptr;
	C_MemAlloc* mem_alloc = nullptr;
	IDirect3DDevice9* direct_device = nullptr;
	IGlowObjectManager* glow_manager = nullptr;
};

#ifdef __READY__
inline C_XorPtr< C_Globals > g_XoredOriginalGlobalsAddress;
#define g_sdk g_XoredOriginalGlobalsAddress.Get( )
#else
inline C_Globals g_sdk = C_Globals( );
inline C_Interfaces g_interfaces = C_Interfaces();
#endif

#define TIME_TO_TICKS( flTime ) ( ( int32_t )( 0.5f + ( float_t )( ( flTime ) ) / g_interfaces.globals->m_intervalpertick ) )
#define TICKS_TO_TIME( iTick ) ( float_t )( ( iTick ) * g_interfaces.globals->m_intervalpertick )
