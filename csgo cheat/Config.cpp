...
void C_ConfigSystem::SaveConfig(const char* szConfigName)
{
	...
	for (int i = 0; i < g_cfg->m_aEquipment.size(); i++)
		jManual[_S("Equipment")][std::to_string(i)] = g_cfg->m_aEquipment[i];

	for (int i = 0; i < g_cfg->m_aWorldRemovals.size(); i++)
		jManual[_S("Removals")][std::to_string(i)] = g_cfg->m_aWorldRemovals[i];

	for (int i = 0; i < g_cfg->m_aInverterConditions.size(); i++)
	{
		jManual[_S("Inverters")][std::to_string(i)] = g_cfg->m_aInverterConditions[i];
		jManual[_S("Fakelags")][std::to_string(i)] = g_cfg->m_aFakelagTriggers[i];
	}

	for (int i = 0; i < g_cfg->m_aChamsSettings.size(); i++)
	{
		jManual[_S("Chams settings")][std::to_string(i)][_S("Render chams")] = g_cfg->m_aChamsSettings[i].m_bRenderChams;
		jManual[_S("Chams settings")][std::to_string(i)][_S("Main material")] = g_cfg->m_aChamsSettings[i].m_iMainMaterial;

		for (int j = 0; j < 4; j++)
		{
			jManual[_S("Chams settings")][std::to_string(i)][std::to_string(j)] = g_cfg->m_aChamsSettings[i].m_Color[j];

			jManual[_S("Chams settings")][std::to_string(i)][_S("Modifiers")][std::to_string(j)] = g_cfg->m_aChamsSettings[i].m_aModifiers[j];
			SET_COLOR(jManual[_S("Chams settings")][std::to_string(i)][_S("Modifiers Colors")][std::to_string(j)], g_cfg->m_aChamsSettings[i].m_aModifiersColors[j]);
		}
	}

	for (int i = 0; i < g_cfg->m_aRageSettings.size(); i++)
	{
		jManual[_S("Rage settings")][std::to_string(i)][_S("Enabled")] = g_cfg->m_aRageSettings[i].m_bEnabled;
		jManual[_S("Rage settings")][std::to_string(i)][_S("Prefer safe")] = g_cfg->m_aRageSettings[i].m_bPreferSafe;
		jManual[_S("Rage settings")][std::to_string(i)][_S("Prefer body")] = g_cfg->m_aRageSettings[i].m_bPreferBody;
		jManual[_S("Rage settings")][std::to_string(i)][_S("Autostop")] = g_cfg->m_aRageSettings[i].m_bAutoStop;
		jManual[_S("Rage settings")][std::to_string(i)][_S("Autoscope")] = g_cfg->m_aRageSettings[i].m_bAutoScope;
		jManual[_S("Rage settings")][std::to_string(i)][_S("Mindamage")] = g_cfg->m_aRageSettings[i].m_iMinDamage;
		jManual[_S("Rage settings")][std::to_string(i)][_S("Mindamage override")] = g_cfg->m_aRageSettings[i].m_iMinDamageOverride;
		jManual[_S("Rage settings")][std::to_string(i)][_S("Hitchance")] = g_cfg->m_aRageSettings[i].m_iHitChance;
		jManual[_S("Rage settings")][std::to_string(i)][_S("Head scale")] = g_cfg->m_aRageSettings[i].m_iHeadScale;
		jManual[_S("Rage settings")][std::to_string(i)][_S("Body scale")] = g_cfg->m_aRageSettings[i].m_iBodyScale;

		for (int j = 0; j < 6; j++)
		{
			jManual[_S("Rage settings")][std::to_string(i)][_S("Hitboxes")][std::to_string(j)] = g_cfg->m_aRageSettings[i].m_Hitboxes[j];
			jManual[_S("Rage settings")][std::to_string(i)][_S("Multipoints")][std::to_string(j)] = g_cfg->m_aRageSettings[i].m_Multipoints[j];
		}

		for (int j = 0; j < 2; j++)
			jManual[_S("Rage settings")][std::to_string(i)][_S("Autostop options")][std::to_string(j)] = g_cfg->m_aRageSettings[i].m_AutoStopOptions[j];

		for (int j = 0; j < 2; j++)
			jManual[_S("Rage settings")][std::to_string(i)][_S("Doubletap options")][std::to_string(j)] = g_cfg->m_aRageSettings[i].m_DoubleTapOptions[j];
	}

	std::ofstream out_file(((std::string)(_S("C:\\reborn hack\\Cfg\\")) + (std::string)(szConfigName)) + ".cfg", std::ios::out | std::ios::trunc);
...
{
	VIRTUALIZER1_START;
	std::filesystem::create_directory("C:\\reborn hack\\Cfg\\");

	std::vector < std::string > m_ConfigList;
	for (const auto& p : std::filesystem::recursive_directory_iterator(_S("C:\\reborn hack\\Cfg\\")))
	{
...
void C_ConfigSystem::LoadConfig(const char* szConfigName)
{
	...
	std::ifstream config_file(((std::string)(_S("C:\\reborn hack\\Cfg\\")) + (std::string)(szConfigName)+".cfg"), std::ios::in);
	if (!config_file.good())
		return;
	...
	nlohmann::json jManual = jConfig[_S("Manual")];
	for (int i = 0; i < g_cfg->m_aEquipment.size(); i++)
		g_cfg->m_aEquipment[i] = jManual[_S("Equipment")][std::to_string(i)].get < bool >();

	for (int i = 0; i < g_cfg->m_aWorldRemovals.size(); i++)
		g_cfg->m_aWorldRemovals[i] = jManual[_S("Removals")][std::to_string(i)].get < bool >();

	for (int i = 0; i < g_cfg->m_aFakelagTriggers.size(); i++)
		g_cfg->m_aFakelagTriggers[i] = jManual[_S("Fakelags")][std::to_string(i)].get < bool >();

	for (int i = 0; i < g_cfg->m_aInverterConditions.size(); i++)
		g_cfg->m_aInverterConditions[i] = jManual[_S("Inverters")][std::to_string(i)].get < bool >();

	for (int i = 0; i < jManual[_S("Chams settings")].size(); i++)
	{
		g_cfg->m_aChamsSettings[i].m_bRenderChams = jManual[_S("Chams settings")][std::to_string(i)][_S("Render chams")].get < bool >();
		g_cfg->m_aChamsSettings[i].m_iMainMaterial = jManual[_S("Chams settings")][std::to_string(i)][_S("Main material")].get < int >();

		for (int j = 0; j < 4; j++)
		{
			g_cfg->m_aChamsSettings[i].m_Color[j] = jManual[_S("Chams settings")][std::to_string(i)][std::to_string(j)].get < int >();

			if (jManual[_S("Chams settings")][std::to_string(i)][_S("Modifiers")].is_object())
			{
				g_cfg->m_aChamsSettings[i].m_aModifiers[j] = jManual[_S("Chams settings")][std::to_string(i)][_S("Modifiers")][std::to_string(j)].get < bool >();
				LOAD_COLOR(jManual[_S("Chams settings")][std::to_string(i)][_S("Modifiers Colors")][std::to_string(j)], g_cfg->m_aChamsSettings[i].m_aModifiersColors[j]);
			}
		}
	}
	...
	for (int i = 0; i < g_cfg->m_aRageSettings.size(); i++)
	{
		g_cfg->m_aRageSettings[i].m_bEnabled = jManual[_S("Rage settings")][std::to_string(i)][_S("Enabled")].get < bool >();
		//g_cfg->m_aRageSettings[ i ].m_bLethalSafety = jManual[ _S( "Rage settings" ) ][ std::to_string( i ) ][ _S( "Lethal safety" ) ].get < bool >( );
		g_cfg->m_aRageSettings[i].m_bPreferSafe = jManual[_S("Rage settings")][std::to_string(i)][_S("Prefer safe")].get < bool >();
		g_cfg->m_aRageSettings[i].m_bPreferBody = jManual[_S("Rage settings")][std::to_string(i)][_S("Prefer body")].get < bool >();
		g_cfg->m_aRageSettings[i].m_bAutoScope = jManual[_S("Rage settings")][std::to_string(i)][_S("Autoscope")].get < bool >();
		g_cfg->m_aRageSettings[i].m_bAutoStop = jManual[_S("Rage settings")][std::to_string(i)][_S("Autostop")].get < bool >();
		if (jManual[_S("Rage settings")][std::to_string(i)][_S("Mindamage")].is_number_integer())
			g_cfg->m_aRageSettings[i].m_iMinDamage = jManual[_S("Rage settings")][std::to_string(i)][_S("Mindamage")].get < int >();
		g_cfg->m_aRageSettings[i].m_iMinDamageOverride = jManual[_S("Rage settings")][std::to_string(i)][_S("Mindamage override")].get < int >();
		g_cfg->m_aRageSettings[i].m_iHitChance = jManual[_S("Rage settings")][std::to_string(i)][_S("Hitchance")].get < int >();
		//g_cfg->m_aRageSettings[ i ].m_iDoubleTapHitChance = jManual[ _S( "Rage settings" ) ][ std::to_string( i ) ][ _S( "DT Hitchance" ) ].get < int >( );
		//g_cfg->m_aRageSettings[ i ].m_iAccuracyBoost = jManual[ _S( "Rage settings" ) ][ std::to_string( i ) ][ _S( "Accuracy" ) ].get < int >( );
		g_cfg->m_aRageSettings[i].m_iHeadScale = jManual[_S("Rage settings")][std::to_string(i)][_S("Head scale")].get < int >();
		g_cfg->m_aRageSettings[i].m_iBodyScale = jManual[_S("Rage settings")][std::to_string(i)][_S("Body scale")].get < int >();

		for (int j = 0; j < 6; j++)
		{
			g_cfg->m_aRageSettings[i].m_Hitboxes[j] = jManual[_S("Rage settings")][std::to_string(i)][_S("Hitboxes")][std::to_string(j)].get < bool >();
			//g_cfg->m_aRageSettings[ i ].m_SafeHitboxes[ j ] = jManual[ _S( "Rage settings" ) ][ std::to_string( i ) ][ _S( "Safe Hitboxes" ) ][ std::to_string( j ) ].get < bool >( );
			g_cfg->m_aRageSettings[i].m_Multipoints[j] = jManual[_S("Rage settings")][std::to_string(i)][_S("Multipoints")][std::to_string(j)].get < bool >();
		}

		for (int j = 0; j < 2; j++)
			g_cfg->m_aRageSettings[i].m_AutoStopOptions[j] = jManual[_S("Rage settings")][std::to_string(i)][_S("Autostop options")][std::to_string(j)].get < bool >();

		for (int j = 0; j < 2; j++)
			g_cfg->m_aRageSettings[i].m_DoubleTapOptions[j] = jManual[_S("Rage settings")][std::to_string(i)][_S("Doubletap options")][std::to_string(j)].get < bool >();
	}
	VIRTUALIZER1_END;
}